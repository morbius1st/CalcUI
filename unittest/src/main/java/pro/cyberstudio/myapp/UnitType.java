...
public enum UnitType {
	...
	private final UnitCategory utUnitCategory;
	...
	UnitType(UnitCategory ucUC, int iUTCode, String sCNS, String sCNP, String sCA, String sAA, double dCF) {
...
//		intUnitTypeCode = signum(iUTCode) * (Math.abs(iUTCode) + ucUC.getCategoryCode());
	 ...
	 */
	private static UnitType getPromoteUnitTypeInternal(UnitType utUT1, int intPromoteLevelAmount) {
		...
		intPromoteUnitTypeCode = (intPrimaryUnitTypeCode - intPrimaryCatCode) +
				(((intPrimaryLevelCode + intPromoteLevelAmount) - 1 ) * UnitCategory.LEVELSEPARATION) +
				intPromoteCatCode;

		// this will return the promoted UnitType or, if one does not exist, "invalid"
		return UnitType.getMemberByCode(intPromoteUnitTypeCode);
	}
	 ...
	 */
	public static UnitType getPromoteUnitType(UnitType utUT1, UnitType utUT2) {
	 ...
	 */
	public static UnitType getPromoteUnitType(UnitType utUT1, int intLC) {
	 ...
	 */
	public static UnitType getDemoteUnitType(UnitType utUT1, int intDemoteLevelAmount) {
		...
		intDemoteUnitTypeCode = (intPrimaryUnitTypeCode - intPrimaryCatCode) +
				((intDemoteUnitTypeCode - 1) * UnitCategory.LEVELSEPARATION) +
				intDemoteCatCode;

		// this will return the promoted UnitType or, if one does not exist, "invalid"
		return UnitType.getMemberByCode(intDemoteUnitTypeCode);
	}
	 ...
	 * Returns UnitType == Invalid for no Demote UnitType
	 */
	public static UnitType getDemoteUnitType(UnitType utUT1, UnitType utUT2) {
	...
	// some units in which the upper case and lower case
	// forms are different units
	public static UnitType getMemberByName(String s) {
		for (UnitType e : UnitType.values()) {
			if ((s.equals(e.getAbbr())) ||
					(s.equalsIgnoreCase(e.getName(SINGULAR))) ||
					(s.equalsIgnoreCase(e.getName(PLURAL))) ||
	...
	public static UnitType getMemberByCode(int c) {
	 ...
	 */
	public static double ConvertUnit(double valueIn, UnitType unitIN, UnitType unitOUT) {
		BigDecimal bdTemp;
